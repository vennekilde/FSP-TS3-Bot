...
public class WebsiteConnector {
    ...
    public String createSession(int tsDbid, String ip, String displayName, boolean isPrimary) {
        String session = null;
        try {
            String url = baseRESTURL + NEW_SESSION_BASE_URL + ACCESS_TOKEN_QUERY_STRING + "&user-id=" + tsDbid + "&ip=" + ip + "&displayname=" + URLEncoder.encode(displayName, "UTF-8") + "&is-primary=" + isPrimary;
